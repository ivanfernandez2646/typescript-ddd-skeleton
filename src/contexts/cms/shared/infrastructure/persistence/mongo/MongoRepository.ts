...
export abstract class MongoRepository<T extends AggregateRoot> {
  ...
  protected async byId(id: string, fromPrimitives: (plainData: unknown) => T) {
    const collection = await this.collection(),
  ...
  protected async byCriteria(criteria: Criteria<unknown>, fromPrimitives: (plainData: unknown) => T): Promise<T[]> {
    const collection = await this.collection(),
      records = await collection
        .find(this.parseCriteria(criteria))
        .sort((criteria?.sort as Sort) ?? {})
  ...
  private parseCriteria(criteria: Criteria<unknown>) {
